...
describe("Testing route GET /tests/teacher", () => {

    it("return 200 - success get tests disicpline", async () => {
        const token = await login();

        const result = await supertest(app).get("/tests/teacher").set({ Authorization: `Bearer ${token}`, Accept: "application/json" }).send();
    ...
    it("return 401 - wrong token", async () => {
        ...
        const result = await supertest(app).get(`/tests/teacher`).set({ Authorization: `Bearer ${token}` }).send();
